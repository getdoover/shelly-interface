...
from .app_ui import ShellyInterfaceUI
from .app_state import ShellyInterfaceState

log = logging.getLogger()

class ShellyInterfaceApplication(Application):
    config: ShellyInterfaceConfig  # not necessary, but helps your IDE provide autocomplete!

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

        self.started: float = time.time()
        self.ui: ShellyInterfaceUI = None
        self.state: ShellyInterfaceState = None

    async def setup(self):
        self.ui = ShellyInterfaceUI()
        self.state = ShellyInterfaceState()
